...
namespace {
    ...
    // PS3 port: parse the string data baked into the binary (strings_data.h)
    // rather than reading strings.json from the filesystem — the console has no
    // host path for the data file.
    json const& string_data() {
        static json const strings = []() {
            try {
                return json::parse(monopoly::kStringsJson);
            }
            catch (json::exception& e) {
                std::cerr << "Exception when parsing baked string data ... " << std::endl;
