...
// main-thread handshake mirrors its frame_mutex / frame_cond pattern. Ported from
// ps3-ray-chess (same RPCS3 lv2 quirks).
#ifdef NETTEST
...
void server_thread(void*) {
    ...
    // The net stack can need a moment to be ready after netInitialize. Retry the
    // whole setup until it binds+listens instead of giving up on the first miss.
    g_listen = -1;
...
void Start() {
    ...
    // RPCS3 lv2 quirks (from ps3-remote-play): the mutex must be NON-adaptive and
    // the cond's attr_pshared must be SYS_COND_ATTR_PSHARED (0 -> CELL_EINVAL).
    // Getting these wrong makes create fail on RPCS3 — so we also degrade gracefully
    // rather than abort if either still fails.
